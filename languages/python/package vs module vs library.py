 ...
 1.Script: is a Python file that’s intended to be run directly. 
 ...
 2.Module:(funtions+global variables)
 ...
 3.Package: (directory having collections of modules.)(modules+_init_.py)
 		    ...
 		    is a collection of related modules that work together to provide certain functionality. 
 ...
 4.Library: A library is an umbrella term referring to a reusable chunk of code.
 			...
 			is an umbrella term that loosely means “a bundle of code.” 
 		 	...
 		 	Matplotlib library:
 		 		is a standard library for generating data visualizations in Python. It supports building basic two-dimensional graphs as well as more complex animated and interactive visualizations.
			PyTorch: 
				is an open-source deep-learning library built by Facebook’s AI Research lab to implement advanced neural networks and cutting-edge research ideas in industry and academia.
			pygame:
				provides developers with tons of convenient features and tools to make game development a more intuitive task.
			Beautiful Soup:
				is a very popular Python library for getting data from the web. The modules and packages inside this library help extract useful information from HTML and XML files.
			Requests:
				 is a part of a large collection of libraries designed to make Python HTTP requests simpler. The library offers an intuitive JSON method that helps you avoid manually adding query strings to your URLs.
			missingno:
				 is very handy for handling missing data points. It provides informative visualizations about the missing values in a dataframe, helping data scientists to spot areas with missing data. It is just one of the many great Python libraries for data cleaning.
			TensorFlow
		  	Scikit-Learn
  		  	Numpy
		  	Keras
...
5. Python Frameworks:
	...
	Also, while libraries contain packages that perform specific operations, 
	...
	Several popular frameworks:	
			Django:
				 is a Python framework for building web applications with less coding. With all the necessary features 	included by default, developers can focus on their applications rather than dealing with routine processes.
			Flask:
				 is a web development framework that is known for its lightweight and modular design. It has many out-of-the-box features and is easily adaptable to specific requirements.
			Bottle:
				 is another lightweight framework for web development that was originally meant for building APIs. Its unique features are that it has no dependencies other than the Python Standard Library and it implements everything in a single source file.
